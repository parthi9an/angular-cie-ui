...
.directive('amMultiselect', ['$parse', '$document', '$compile', '$interpolate', '$filter', 'optionParser',
    ...
    return {
        ...
        scope.selectedObj = attrs.mgSelectedObj || [];
        ...
        function parseModel() {
            ...
            scope.selectedObj = (!(scope.selectedObj instanceof Object)) ? JSON.parse(scope.selectedObj) : scope.selectedObj;
            if(!angular.isDefined(model)) return;
            for (var i = 0; i < model.length; i++) {
                var local = {};
                var checked = false;
                local[parsedResult.itemName] = model[i];
                if(scope.header.match(/select/i) && scope.selectedObj.length > 0 
                		&& scope.selectedObj.indexOf(model[i]) != -1){
                	checked = true;
                }
                scope.items.push({
        ...
        function getHeaderText() {
            ...
            if (isMultiple) {
                if (attrs.msSelected) {
                	scope.selectedObj = JSON.parse(attrs.msSelectedObj);
                	var dummyHeader = scope.selectedObj.length + " " + attrs.msSelected + "" +(scope.selectedObj.length > 1 
                		?  "s" : "") + " selected"; 
                    scope.header = $interpolate(dummyHeader)(scope);
                } else {
...
.directive('amMultiselectPopup', ['$document', '$filter', function ($document, $filter) {
    return {
        ...
        templateUrl: function (element, attr) {
            return attr.templateUrl || 'js/plugins/multiselect/multiselect.tmpl.html';
        },
